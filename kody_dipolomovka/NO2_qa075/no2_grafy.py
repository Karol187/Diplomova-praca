...
# -*- coding: utf-8 -*-
"""
Created on Sun Sep  5 13:41:09 2021
...
for i, row in tabulka.iterrows():   
    ...
    data_2019 = {'Dátum': datumy_2019,'Hodnoty': hodnoty_2019}
    ...
    plt.xlabel('Dátum')
    plt.ylabel("Troposférický stĺpec NO$_2$ počas roka 2019 [mol/m$^2$]")
    plt.xticks(rotation='horizontal')
    #plt.savefig(str(row['name'])+' počas roka 2019.png',bbox_inches='tight',dpi=600)
    #plt.clf()
    plt.show()

    data_2020 = {'Dátum': datumy_2020,'Hodnoty': hodnoty_2020}
    df_2020 = pd.DataFrame(data_2020)
    
    df_2020.plot(kind = 'scatter',
            x = 'Dátum',
            y = 'Hodnoty',
            color = 'red')
    plt.title(str(row['name'])+' počas roka 2020')
    plt.xlabel('Dátum')
    plt.ylabel("Troposférický stĺpec NO$_2$ počas roka 2020 [mol/m$^2$]")
    plt.xticks(rotation='horizontal')
    #plt.savefig(str(row['name'])+' počas roka 2020.png',bbox_inches='tight',dpi=600)
    #plt.clf()
