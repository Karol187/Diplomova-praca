...
%matplotlib inline
df=pd.read_csv('no2_velka_tabulka_2019_index_datum.csv')
df['Datetime']=pd.to_datetime(df['Datetime'])
df = df.set_index(['Datetime'])
df = df.rename(columns=lambda x: x.replace('Hodnoty ', ''))
df_all=df.between_time('00:00:00', '23:59:59').count().to_frame(name='cely cas')
...
plt.title('Počet dát nad UT stanicami, (rok 2019)')
plt.xlabel('Čas')
plt.ylabel('Počet dát')
plt.savefig('pocet_dat_UT_stanice_2019',bbox_inches='tight',dpi=600)
plt.clf()
...
plt.xticks(rotation='vertical')
plt.xticks(rotation='vertical')
plt.title('Počet dát nad UB stanicami, (rok 2019)')
plt.xlabel('Čas')
plt.ylabel('Počet dát')
plt.savefig('pocet_dat_UB_stanice_2019',bbox_inches='tight',dpi=600)
plt.clf()
...
plt.xticks(rotation='vertical')
plt.xticks(rotation='vertical')
plt.title('Počet dát nad RB stanicami, (rok 2019)')
plt.xlabel('Čas')
plt.ylabel('Počet dát')
plt.savefig('pocet_dat_RB_stanice_2019',bbox_inches='tight',dpi=600)
plt.clf()
